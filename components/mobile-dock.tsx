...
export function MobileDock() {
  ...
  return (
    <nav aria-label="Mobile navigation" className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-5 rounded-[24px] border border-[#eadfd7] bg-white/95 p-1.5 shadow-[0_18px_50px_rgba(88,66,51,0.18)] backdrop-blur-xl md:hidden">
      <Link href="/" aria-current={current("/")} className={itemClass("/")}><span className="text-base" aria-hidden>⌂</span><span>Home</span></Link>
      <Link href="/shop" aria-current={current("/shop")} className={itemClass("/shop")}><span className="text-base" aria-hidden>⌕</span><span>Discover</span></Link>
      <Link href="/wishlist" aria-current={current("/wishlist")} className={`${itemClass("/wishlist")} relative`} aria-label={`Saved items${wishlist.length ? `, ${wishlist.length}` : ""}`}><span className="text-base" aria-hidden>♡</span><span>Saved</span>{wishlist.length ? <span aria-hidden className="absolute right-2 top-1 grid h-4 min-w-4 place-items-center rounded-full bg-orange-500 px-1 text-[8px] text-white">{wishlist.length}</span> : null}</Link>
      <Link href="/cart" aria-current={current("/cart")} className={`${itemClass("/cart")} relative`} aria-label={`Cart${cartCount ? `, ${cartCount} items` : ""}`}><span className="text-base" aria-hidden>◫</span><span>Cart</span>{cartCount ? <span aria-hidden className="absolute right-2 top-1 grid h-4 min-w-4 place-items-center rounded-full bg-orange-500 px-1 text-[8px] text-white">{cartCount}</span> : null}</Link>
      <Link href={accountHref} aria-current={current(accountHref)} className={itemClass(accountHref)}><span className="text-base" aria-hidden>○</span><span>{currentUser ? "Account" : "Sign in"}</span></Link>
    </nav>
  );
}
