    ...
    <header className="sticky top-0 z-40 border-b border-[#eadfd7]/90 bg-[#fffaf5]/95 backdrop-blur-xl">
      ...
      <div className="mx-auto flex max-w-[1480px] flex-wrap items-center gap-2.5 px-3 py-2.5 sm:gap-3 md:flex-nowrap md:px-6 md:py-3">
        ...
        <form action="/shop" method="get" className="order-3 flex min-w-0 basis-full items-center rounded-[20px] border border-[#e8ddd5] bg-white p-1.5 shadow-[0_8px_26px_rgba(88,66,51,0.06)] focus-within:border-orange-300 focus-within:ring-4 focus-within:ring-orange-100 md:order-none md:mx-auto md:max-w-2xl md:flex-1 md:basis-auto">
          <span className="pl-3 text-[#66574d]">⌕</span>
          <input name="q" type="search" placeholder="Find products, sellers or locations" className="min-w-0 flex-1 bg-transparent px-2.5 py-2 text-[16px] text-[#2b211c] outline-none placeholder:text-[#75655b] sm:px-3 sm:text-sm" />
        ...
        <nav className="ml-auto flex shrink-0 items-center gap-1.5 text-xs font-bold text-[#594b42] sm:gap-2">
          <Link href={currentUser ? "/account" : "/login"} className="hidden rounded-2xl px-3 py-2 hover:bg-white hover:text-[#a63d08] lg:block">
          ...
          <Link href="/wishlist" className="relative hidden h-10 w-10 place-items-center rounded-2xl bg-white text-lg text-[#493a31] shadow-sm ring-1 ring-[#eadfd7] hover:text-[#a63d08] sm:grid" aria-label="Wishlist">
            ♡
          ...
          <Link href="/cart" className="relative flex h-10 items-center gap-2 rounded-2xl bg-[#2b211c] px-3 text-white shadow-sm hover:bg-orange-600">
            ...
            {cartCount ? <span className="rounded-full bg-white/20 px-1.5 py-0.5 text-[10px] text-white">{cartCount}</span> : null}
          </Link>
        </nav>
      </div>
      ...
      <div className="border-t border-[#eee3dc] bg-white/80">
        <nav className="mx-auto flex max-w-[1480px] items-center gap-2 overflow-x-auto px-3 py-2 text-xs font-bold [scrollbar-width:none] md:px-6 [&::-webkit-scrollbar]:hidden">
          ...
          <Link href="/deals" className="shrink-0 rounded-full bg-orange-50 px-4 py-2 text-[#a63d08] ring-1 ring-orange-100 hover:bg-orange-100">Deals</Link>
          <Link href="/categories" className="shrink-0 rounded-full border border-[#eadfd7] bg-white px-4 py-2 text-[#493a31] hover:border-orange-200 hover:text-[#a63d08]">Categories</Link>
          ...
          <Link href="/sellers" className="shrink-0 rounded-full border border-[#eadfd7] bg-white px-4 py-2 text-[#493a31] hover:border-orange-200 hover:text-[#a63d08]">Sellers</Link>
          <Link href="/help" className="shrink-0 rounded-full border border-[#eadfd7] bg-white px-4 py-2 text-[#493a31] hover:border-orange-200 hover:text-[#a63d08]">Help</Link>
          <Link href="/seller" className="ml-auto hidden shrink-0 rounded-full bg-[#f8efe8] px-4 py-2 text-[#9a3a0a] hover:bg-orange-100 md:block">Seller studio →</Link>
        </nav>
      </div>
    </header>
  );
